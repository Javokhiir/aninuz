...
@section('content')
<form action="{{dashboard_route('dashboard.products.update', ['product' => $item->id])}}" method="post" class="productForm" enctype="multipart/form-data">
    @csrf
     @method('PUT')
    <div class="col">
        <div class="card">
                                ...
                                <div class="card-body">
                                    <div class="row row-cols-1 row-cols-md-3 g-3">
                                        ...
                                        <div class="col">
                                            ...
                                            <div class="form-floating">
                                                <input type="text" class="form-control" id="model" name="articul" placeholder="articul" value="{{old('articul', $item->articul)}}">
                                            ...
                                            <div class="form-floating">
                                                <select class="form-select custom-select" id="brand" name="brand" aria-label="Floating label select example">
                                                    ...
                                                    @foreach ($brands as $brand)
                                                        <option value="{{$brand->slug}}" @if(old('brand',$item->brand) == $brand->slug) selected @endif>{{$brand->title}}</option>
                                                    @endforeach
                                                </select>
                                            </div>
                                        </div>
                                            ...
                                            <div class="my-3">
                                                ...
                                                <div class="form-floating">
                                                    <textarea class="form-control editor" placeholder="Leave a description here" id="table_content_{{$locale}}" name="{{$locale}}[table_content]" style="height: 150px">
                                                        {{ old("$locale.table_content",$item->{'table_content:'.$locale }) }}
                                                    </textarea>
                                                </div>
                                            </div>
                                            <div>
                                                ...
                                                <div class="form-floating">
                                                    <textarea class="form-control editor" placeholder="Leave a description here" id="table_content_second_{{$locale}}" name="{{$locale}}[table_content_second]" style="height: 150px">
                                                        {{ old("$locale.table_content_second",$item->{'table_content_second:'.$locale }) }}
                                                    </textarea>
                                                </div>
                                            </div>
                                        </div>
                                        ...
                                        <div class="col">
                                            ...
                                            <div id="uploadedFiles">
                                                ...
                                                <div class="file">
                                                    ...
                                                    <button type="button" class="fileDelete uploadedFileDelete"
                                                        data-url={{dashboard_route('dashboard.products.delete_image')}}
                                                        data-product="{{$item->id}}" 
                                                        data-image="{{$pic->id}}"><span class="icon"><i class="bi bi-trash"></i></span></button>
                                                </div>
                                                @endforeach
                                            </div>
                                        </div>
                                            ...
                                            <select class="form-select custom-select" id="categories" name="categories[]" multiple>
                                                @foreach ($categories as $category)
                                                    <option value="{{$category->id}}" 
                                                        @if(in_array($category->id, old('categories',$item->categories->pluck('id')->toArray()))) selected="selected" @endif>
                                                        @if($parent = $category->parent) {{$parent->title}}: @endif{{$category->title}}
                                                    </option>
                                                @endforeach
                                            </select>
                                        </div>
                    ...
                    <div class="card card-form">
                        <div class="card-header">
                            <div class="d-flex justify-content-between">
                                ...
                                <button type="button" class="btn btn-primary" id="addFaq" 
                                    data-row="{{$faq_count-1}}" data-url="{{dashboard_route('dashboard.products.faq_component')}}">
                                    Add
                                </button>
                            </div>
                        </div>
                        <div class="card-body">
                            <div class="accordion" id="accordionFAQ">
                                ...
                                <div class="accordion-item">
                                    <h2 class="accordion-header">
                                        <button class="accordion-button" type="button" data-bs-toggle="collapse" data-bs-target="#collapseFAQ_{{$key}}" aria-expanded="true" aria-controls="collapseFAQ_{{$key}}">
                                            FAQ #{{$key+1}}
                                        </button>
                                    </h2>
                                    <div id="collapseFAQ_{{$key}}" class="accordion-collapse collapse" data-bs-parent="#accordionFAQ">
                                        <div class="accordion-body">
                                            <nav>
                                                <div class="nav nav-tabs" id="nav-tab" role="tablist">
                                                    @foreach ($locales as $locale)
                                                    <button class="nav-link @if($selected_locale === $locale) active @endif" id="nav-faq-{{$locale}}-tab_{{$key}}" data-bs-toggle="tab" 
                                                        data-bs-target="#nav-faq-{{$locale}}_{{$key}}" type="button" role="tab" aria-controls="nav-faq-{{$locale}}_{{$key}}" 
                                                        aria-selected="true">@lang("admin.$locale")</button>
                                                    @endforeach
                                                </div>
                                            </nav>
                                            <div class="tab-content" id="nav-tabContent">
                                                @foreach ($locales as $locale)
                                                <div class="tab-pane fade @if($selected_locale === $locale) show active @endif" id="nav-faq-{{$locale}}_{{$key}}" role="tabpanel" aria-labelledby="nav-faq-{{$locale}}-tab_{{$key}}" tabindex="0">
                                                    <div class="my-3">
                                                        <label for="title_{{$locale}}" class="form-label">@lang("admin.title") <span class="important">*</span></label>
                                                        <div class="form-floating">
                                                            <input type="text" class="form-control" id="title_{{$locale}}" 
                                                                name="faqs[{{$key}}][{{$locale}}][title]" placeholder="@lang("admin.title")" 
                                                                value="{{ $faq->{'title:'.$locale } }}" >
                                                            <label for="title_{{$locale}}">@lang("admin.title")</label>
                                                        </div>
                                                    </div>
                                                    <div>
                                                        ...
                                                        <div class="form-floating">
                                                            <textarea class="form-control" placeholder="Leave a description here" id="content_{{$locale}}" 
                                                                name="faqs[{{$key}}][{{$locale}}][content]" style="height: 150px">{{ $faq->{'content:'.$locale } }}</textarea>
                                                        </div>
                                                    </div>
                                                </div>
                                                @endforeach
